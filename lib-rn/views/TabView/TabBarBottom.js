...
import { Animated, TouchableWithoutFeedback, StyleSheet, View, Platform, Keyboard, Dimensions } from 'react-native';
...
const isTablet = Dimensions.get('window').height / Dimensions.get('window').width < 1.6;
...
class TabBarBottom extends React.PureComponent {
  ...
  _renderLabel = scene => {
    ...
    if (typeof label === 'string') {
      return <Animated.Text style={[styles.label, { color }, showIcon && this._shouldUseHorizontalTabs() ? styles.labelBeside : styles.labelBeneath, labelStyle]} allowFontScaling={allowFontScaling}>
          {label}
        </Animated.Text>;
    }
  ...
  _renderIcon = scene => {
    ...
    return <TabBarIcon position={position} navigation={navigation} activeTintColor={activeTintColor} inactiveTintColor={inactiveTintColor} renderIcon={renderIcon} scene={scene} style={showLabel && this._shouldUseHorizontalTabs() ? {} : styles.icon} />;
  };
  ...
  render() {
    ...
    const isHeightConstrained = layout.height === 0 ? !isTablet : layout.height < 500;
    const tabBarStyle = [styles.tabBar, this._shouldUseHorizontalTabs() && isHeightConstrained ? styles.tabBarCompact : styles.tabBarRegular, style];
        ...
        <SafeAreaView style={tabBarStyle} forceInset={{ bottom: 'always', top: 'never' }}>
          ...
          return <TouchableWithoutFeedback key={route.key} testID={testID} accessibilityLabel={accessibilityLabel} onPress={() => onPress ? onPress({ previousScene, scene, jumpToIndex }) : jumpToIndex(index)}>
                <Animated.View style={[styles.tab, { backgroundColor }]}>
                  <View style={[styles.tab, this._shouldUseHorizontalTabs() ? styles.tabLandscape : styles.tabPortrait, tabStyle]}>
